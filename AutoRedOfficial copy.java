...
public class AutoRedOfficial extends OpMode {
    ...
    public void loop() {
        ...
        if(this.time > 0 && this.time <=6.00){ //wait 6 sec
            ...
            right = 0.3;
            intake = -1;
        } else if (this.time > 6.50 && this.time <= 7.5){ //turn 45 degrees right //1
            left = -0.8;
            right = 0.8;
            intake = -1;
        }
        else if(this.time > 7.5 && this.time <= 10.4){ //go forward to floor goal
            left = 0.3;
            right = 0.3;
            intake = -1;
        }
